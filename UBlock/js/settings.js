...
function handleImportFilePicker() {
  ...
  const expectedFileTypes = ["text/plain", "application/json"];
  ...
  fr.onload = function () {
    ...
    const msg = i18n$("aboutRestoreDataConfirm").replace(
      "{{time}}",
      time.toLocaleString(),
    );
...
function onLocalDataReceived(details) {
  ...
  if (lastBackupFile !== "") {
    ...
    node.textContent =
      text + "\xA0" + dt.toLocaleString("fullwide", timeOptions);
    node.style.display = "";
  }
...
function onUserSettingsReceived(details) {
  ...
  if (details.canLeakLocalIPAddresses === true) {
    qs$('[data-setting-name="webrtcIPAddressHidden"]').closest(
      "div.li",
    ).style.display = "";
  }

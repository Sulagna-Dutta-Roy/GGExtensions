 ...
 * @author Krcma
 */
public class MediumBlock implements Block, Serializable {
    ...
    private BlockType TYPE = BlockType.MEDIUM;

    public MediumBlock(Point offset_position, Color color) {
    ...
    public void render(Graphics2D g2, boolean fromDown, boolean upper, int x_off, int y_off) {
        if (!upper) {
            ...
            //**********************
            for (int i = 3; i <= 3; i++) {
                g2.drawRect(x + i, y + i, Block.SIZE - 2 * i, Block.SIZE - 2 * i);
            }

 ...
 * @author Krcma
 */
public class DoubleLaserGunBlock implements Block, Serializable {
    ...
    private final BlockType TYPE = BlockType.DOUBLE_LASER_GUN;

    public DoubleLaserGunBlock(Point offset_position, Color color) {
    ...
    public void render(Graphics2D g2, boolean fromDown, boolean upper, int x_off, int y_off) {
        ...
        if (!upper) {
            ...
            if (fromDown) {
                g2.drawLine(
                        (int) (x + Block.SIZE * 0.3f),
                        (int) (y + Block.SIZE * 0.5f),
                        (int) (x + Block.SIZE * 0.3f),
                        (int) (y - Block.SIZE * 0.3f)
                );
                g2.drawLine(
                        (int) (x + Block.SIZE * 0.7f),
                        (int) (y + Block.SIZE * 0.5f),
                        (int) (x + Block.SIZE * 0.7f),
                        (int) (y - Block.SIZE * 0.3f)
                );

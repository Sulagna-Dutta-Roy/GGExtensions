 ...
 * @author Krcma
 */
public class EngineBlock implements Block, Serializable {
    ...
    private final BlockType TYPE = BlockType.ENGINE_BLOCK;

    public EngineBlock(Point offset_position, Color color) {
    ...
    public void render(Graphics2D g2, boolean fromDown, boolean upper, int x_off, int y_off) {
        if (!upper) {
            ...
            float step = (Block.SIZE * 0.7f) / 4f;
            for (int i = 0; i < 5; i++) {
                g2.drawLine(
                        (int) (x + Block.SIZE * 0.2f),
                        (int) (y + Block.SIZE * 0.15f + i * step),
                        (int) (x + Block.SIZE * 0.8f),
                        (int) (y + Block.SIZE * 0.15f + i * step)
                );
            }

document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (
      ...
      !burstTimeInput.value.trim() ||
      (algo === "RR" && !timeQuantumInput.value.trim())
    ) {
    ...
    if (
      arrivalTime.includes(NaN) ||
      burstTime.includes(NaN) ||
      (algo === "RR" && isNaN(timeQuantum))
    ) {
    ...
    while (processesInfo.some((process) => process.remainingBurstTime > 0)) {
      for (const process of processesInfo) {
        ...
        if (process.remainingBurstTime <= 0) {
          ...
          process.waitingTime =
            process.turnaroundTime - burstTime[process.processId - 1];
